...
query_javascript_request = 'SELECT script_url, visit_id FROM javascript'

def fetch_data(query_type,db_path):
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    c.execute(query_javascript_request)
...
def append_third_parties(website_arr, third_party_arr, row_results):
    for result in row_results:
        ...
        if check_third_party(script_url, sites[visit_id]):
...
if __name__ == "__main__":
    ...
    vanilla_data_rows = fetch_data(query_javascript_request, VANILLA_DB)
    ad_block_data_rows = fetch_data(query_javascript_request, ADBLOCK_DB)
    ...
    # Vanilla Mode
    plt.title("Third-Party JavaScript API Calls(Vanilla/Ad-Block)")
    plt.xlabel("Top 100 Websites")
    plt.ylabel("# of Third-Party JS API Calls")
